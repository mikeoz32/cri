  ...
  module Tui
    ...
    class EventHandler
      ...
      private def submit_now(command_mode : Bool, refresh : Proc(Nil)) : Bool
        ...
        if command_mode && (target = begin_interactive_auth(raw))
          ...
          spawn do
            begin
              provider_id, flow_id, kind = target
              ref = if kind == Auth::FlowKind::OAuthBrowser
                      controller.host.login_browser(provider_id, flow_id) { |status| ui.set_activity("#{status}\n", "activity") }
                    else
                      controller.host.login_device(provider_id, flow_id) { |status| ui.set_activity("#{status}\n", "activity") }
                    end
      ...
      private def begin_interactive_auth(raw : String) : {String, String, Auth::FlowKind}?
        parts = raw.split
        ...
        return nil unless {Auth::FlowKind::OAuthBrowser, Auth::FlowKind::OAuthDevice}.includes?(flow.kind)
        {provider.id, flow.id, flow.kind}
      end
